...
class CommandInterpreter {
    ...
    private void loadStatement() {
        ...
        String tname = name();
        _input.next(";");
        Table table = Table.readTable(tname);
        System.out.println("Loaded " + tname + ".db");
        _database.put(tname, table);
    }

    /** Parse and execute a store statement from the token stream. */
    private void storeStatement() {
        _input.next("store");
        String tname = _input.peek();
        Table table = tableName();
        _input.next(";");
        table.writeTable(tname);
        System.out.println("Stored " + tname + ".db");
    }
    ...
    private void selectStatement() {
        Table table = selectClause("tempTable");
        _input.next(";");
    ...
    Table tableDefinition(String name) {
        if (_input.nextIf("(")) {
            _createColTitles = new ArrayList<>();
            _createColTitles.add(name());
            while (_input.nextIf(",")) {
                _createColTitles.add(name());
            }
            _input.next(")");
            return new Table(name, _createColTitles);
    ...
    Table selectClause(String tableName) {
        ...
        _selectedColumns = new ArrayList<Column>();
        _tableIter = new ArrayList<TableIterator>();
        ArrayList<String> colTitles =
            new ArrayList<>();

        _selectedColumns.add(columnSelector());
        while (_input.nextIf(",")) {
            _selectedColumns.add(columnSelector());
        }

        _input.next("from");
        _tableIter.add(tableName().tableIterator());
        if (_input.nextIf(",")) {
            _tableIter.add(tableName().tableIterator());
        }

        for (Column column : _selectedColumns) {
            column.resolve(_tableIter);
            colTitles.add(column.specifiedName());
        }

        _conditions = conditionClause(_tableIter);

        Table table = new Table(tableName, colTitles);
        select(table, _selectedColumns, _tableIter, _conditions);
    ...
    private void select(Table table, ArrayList<Column> columns,
        ...
        if (iterators.size() > 2) {
            throw error("Wrong number of iterators.");
        }
        TableIterator iter1 = iterators.get(0);
        TableIterator iter2 = null;

        if (iterators.size() == 1) {
            oneTableSelect(table, columns, iter1, conditions);
        } else {
            iter2 = iterators.get(1);
            twoTableSelect(table, columns, iter1, iter2, conditions);
        }
    }

    /** Takes TABLE and selects COLUMNS iterated by ITER1 that satisfy
     *  all CONDITIONS. A single-table select with or without conditions. */
    void oneTableSelect(Table table, ArrayList<Column> columns,
                        TableIterator iter1,
                        List<Condition> conditions) {
        while (iter1.hasRow()) {
    ...
    /** Takes TABLE and selects COLUMNS iterated by ITER1 and ITER2 that satisfy
     *  all CONDITIONS. A two-table select with or without conditions. */
    void twoTableSelect(Table table, ArrayList<Column> columns,
                        TableIterator iter1, TableIterator iter2,
                        List<Condition> conditions) {
        while (iter1.hasRow()) {
            if (iter2 == null) {
                if (Condition.test(conditions)) {
    ...
    /** My column titles during a create statement. */
    private ArrayList<String> _createColTitles;
    /** My columns selected during a select clause. */
    private ArrayList<Column> _selectedColumns;
    /** My conditions during a select clause. */
    private List<Condition> _conditions;
    /** My table iterators in a select clause. */
    private List<TableIterator> _tableIter;
